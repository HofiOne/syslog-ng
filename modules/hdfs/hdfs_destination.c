...
void
hdfs_destination_dd_set_filename(LogDriver *d, const gchar *filename)
{
  HDFSDestinationDriver *self = (HDFSDestinationDriver *)d;
...
_format_stats_key(LogThreadedDestDriver *d, StatsClusterKeyBuilder *kb)
{
  ...
  stats_cluster_key_builder_add_legacy_label(kb, stats_cluster_label("driver", "hdfs-destination"));
...
_format_persist_name(const LogPipe *d)
{
  ...
  if (d->persist_name)
    g_snprintf(persist_name, sizeof(persist_name), "hdfs-destination.%s", d->persist_name);
  else
    g_snprintf(persist_name, sizeof(persist_name), "hdfs-destination.%s", self->filename->str);
...
LogDriver *
hdfs_destination_dd_new(GlobalConfig *cfg)
{
  HDFSDestinationDriver *self = g_new0(HDFSDestinationDriver, 1);
  ...
  self->super.stats_source = stats_register_type("hdfs-destination");
  self->super.worker.construct = hdfs_destination_dw_new;
