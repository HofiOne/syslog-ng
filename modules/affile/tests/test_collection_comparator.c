...
ParameterizedTest(const struct TestFileList *tup, collection_comparator, test_collection_changes)
{
  ...
  cr_assert(ndx == 0 || ndx == 4 ? g_hash_table_size(data->deleted_entries) == 0 : g_hash_table_size(data->deleted_entries) > 0);
  ...
  cr_assert(ndx != 4 && ndx != 5 ? g_hash_table_size(data->new_entries) == 0 : g_hash_table_size(data->new_entries) > 0);
