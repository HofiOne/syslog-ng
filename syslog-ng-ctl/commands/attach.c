...
GOptionEntry attach_options[] =
{
  ...
  { "fds-to-steel", 0, 0, G_OPTION_ARG_CALLBACK, _parse_fd_names, "which stdio file handlers to attach to, default is <stdout,stderr>, valid only with the `stdio` attach mode", "comma separated list of <stdin|stdout|stderr>"  },
...
slng_attach(int argc, char *argv[], const gchar *mode, GOptionContext *ctx)
{
  ...
  g_string_append_printf(command, " %d", attach_options_fds_to_steel > 0 ? attach_options_fds_to_steel : (1 << STDOUT_FILENO) | (1 << STDERR_FILENO));
  if (attach_options_log_level)
